...
for i = 1 : length(time)
    ...
    %慣性座標系におけるx,yの目標との偏差
        ...
        % 詳細なパラメータについて定義してるだけ
        dphi{iu} = p{iu}(i,1);                                     %4
        dp{iu} = Mc*delta_a{iu}(i,1)/Ixx;                          %5

        % ここまで出てきた状態変数の時系列データ格納用
        x_e_vec{iu}(i,:) = [dx_e{iu};dy_e{iu};dchi_e{iu};dot_s{iu};0];

        % セレ・フレネ座標系と慣性座標系の間の角速度の変換
        dchi{iu} = - dchi_e{iu} + dchi_d{iu}(i,1) ;  % 航路角速度 [rad/s]
        GammaChi{iu}(i,2) = dchi{iu};   % 保存用
        dpsi{iu} = V_g{iu}(i,1)*dchi{iu} / (V_a*cos(chi{iu}-psi{iu}));  % ヨー角速度 [rad/s]
        ...
        % dx = V_a*cos(psi) + Wx;
        % dy = -V_a*sin(psi) + Wy;
        dx{iu} = V_g_vec{iu}(1,1);
        dy{iu} = V_g_vec{iu}(2,1);
        ...
        X_I{iu}(i+1,1) = X_I{iu}(i,1) + dx{iu} * dt;  % 位置 x [m]の更新
        X_I{iu}(i+1,2) = X_I{iu}(i,2) + dy{iu}*dt;  % 位置 y [m]の更新
        X_I{iu}(i+1,3) = X_I{iu}(i,3) + dpsi{iu}*dt;  % ヨー角 psi [rad]の更新
        while (X_I{iu}(i+1,3) < -pi || X_I{iu}(i+1,3) >= pi) % 0 <= psi < 2*pi の否定
            if (X_I{iu}(i+1,3) > pi)
        ...
        X_I{iu}(i+1,4) = X_I{iu}(i,4) + dot_s{iu}*dt;  % 経路長 s [m]の更新
        

        phi{iu}(i+1,1) = phi{iu}(i,1) + dphi{iu}*dt;   % ロール角 Φ[rad]の更新
        
        % ロール角の上限下限値を設定
        if phi{iu}(i+1,1) > 70*pi/180
            phi{iu}(i+1,1) = 70*pi/180;
        elseif phi{iu}(i+1,1) < -70*pi/180
            phi{iu}(i+1,1) = -70*pi/180;
        end

        % phi（ロール）の微分値(角速度)pの更新
        p{iu}(i+1,1) =p{iu}(i,1) + dp{iu}*dt;
        
        % ロール角速度の上限下限値を設定
        if p{iu}(i+1,1) > 100*pi/180
            p{iu}(i+1,1) = 100*pi/180;
        elseif p{iu}(i+1,1) < -100*pi/180
            p{iu}(i+1,1) = -100*pi/180;
        end
        
        % xi 計算用の変数保存
        Cxi{iu}(i+1,:) = [s{iu} + dot_s{iu}*dt, dot_s{iu}, t, dt, s{iu}, F{iu}(i,6), F{iu}(i,7)];
    end
end
...
for i=1:2
    ...
    p1 = plot(F{i}(:,1),F{i}(:,2),'g-','linewidth',4);  % 目標経路
    p2 = plot(X_I{i}(1,1),X_I{i}(1,2),'mo','linewidth',6);  % 初期位置
    p3 = plot(X_I{i}(end,1),X_I{i}(end,2),'bo','linewidth',4);  % 最終位置
    p4 = plot(X_I{i}(:,1),X_I{i}(:,2),'b','linewidth',2);  % 飛行経路
    axis equal
    %('目標経路と飛行経路')
